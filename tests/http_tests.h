...
void test_make_http_request(void)
{
   ...
   result = make_http_request(requestUrl, requestBody, method, headers, timeout,
         timeout);
...
void test_GET_http_post_NOT_NULL_headers(void)
{
   ...
   http_method_t method = http_method_t::GET;
   char* requestBody = const_cast<char*>("");
   char* requestUrl = const_cast<char*>("http://127.0.0.1:8080/test_get_method");
...
void test_check_request_headers(void)
{
   ...
   CU_ASSERT_PTR_NOT_NULL(
         strstr(response->response_body, "Content-Type: application/json"));
}
